...
export default function AdminCMS({ language, onClose }: AdminCMSProps) {
  ...
  const [activeSubBlock, setActiveSubBlock] = useState<'intro' | 'founder' | 'belief' | 'cta' | 'portals'>('intro');
  // Selected gallery type in Tab 7
  const [activeGalleryType, setActiveGalleryType] = useState<'ml' | 'rh' | 'cl'>('ml');
  ...
  useEffect(() => {
    ...
    // 4. Homepage Blocks (CMS text config docs: intro_banner, statistics, founder, central_belief, cta_block, gallery_ml, gallery_rh, gallery_cl)
    const unsubBlocks = onSnapshot(collection(db, 'homepage_blocks'), (snapshot) => {
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (doc.id === 'intro_banner') setIntroBlock(data);
    ...
    <div className="fixed inset-0 z-[9999] bg-[#FAF7F2] flex flex-col font-serif select-text overflow-hidden bg-grain">
      ...
      {!isAuthenticated ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <nav className="w-56 bg-stone-100 border-r border-[#E8DDD0] flex flex-col justify-between shrink-0 font-sans">
            <div className="p-4 space-y-1">
              <div className="text-[9px] font-extrabold text-[#B8862A] uppercase tracking-wider pb-3 border-b border-[#E8DDD0] mb-3">
                {language === 'bn' ? 'মডিউলসমূহ' : 'CMS Sections'}
              </div>

              {/* Tab 1 */}
              <button
                onClick={() => { setActiveTab('hero'); setEditingHero(null); }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold leading-none tracking-wide transition cursor-pointer text-left ${
                  activeTab === 'hero' 
              ...
              >
                ...
                <span>{language === 'bn' ? '১. প্রথম স্লাইডার' : '1. Top Slider'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '২. আসন্ন কার্যক্রমসমূহ' : '2. Upcoming Activities'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '৩. হোমপেজ কন্টেন্ট ব্লক' : '3. Homepage Blocks'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '৪. মূল পরিসংখ্যান' : '4. Key Statistics'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '৫. কার্যক্রমসমূহ (slider)' : '5. Programs Slider'}</span>
              </button>
              ...
              >
                <ImageIcon className="h-4 w-4" />
                <span>{language === 'bn' ? '৬. অন্যান্য গ্যালারি' : '6. Homepage Galleries'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '৭. দেশব্যাপী আন্দোলন ও CTA' : '7. Movement & CTA'}</span>
              </button>

              {/* Tab 8: About & Management */}
              <button
              ...
              >
                ...
                <span>{language === 'bn' ? '৮. পরিচিতি ও ব্যবস্থাপনা' : '8. About & Management'}</span>
              </button>
              
              {/* Tab 9: Programs Page Contents */}
              <button
              ...
              >
                <BookOpen className="h-4 w-4" />
                <span>{language === 'bn' ? '৯. পরিষেবা, ভবন ও কার্যক্রম' : '9. Services, Building & Programs'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '১০. তথ্যকেন্দ্র ও নোটিশ বোর্ড' : '10. Notice Board'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '১১. যোগাযোগ ও বার্তা' : '11. Contact & Inquiries'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '১২. নিয়োগ বিজ্ঞপ্তি' : '12. Careers & Niyog'}</span>
              </button>
              ...
              >
                ...
                <span>{language === 'bn' ? '১৩. প্রেস ও মিডিয়া' : '13. Press & Media'}</span>
              </button>
                  ...
                  <div className="space-y-4">
                    {!editingPage ? (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { id: 'auditorium', name_bn: 'অডিটোরিয়াম ও সেমিনার কক্ষ', name_en: 'Auditoriums & Halls' },
                            { id: 'cafe', name_bn: 'ক্যাফেটেরিয়া ও ফুড মেনু', name_en: 'BSK Book Cafe' },
                            { id: 'publication', name_bn: 'প্রকাশনা ও প্রকাশনী', name_en: 'Publications' },
                            { id: 'bookshop', name_bn: 'বই বিক্রয় কেন্দ্র', name_en: 'BSK Book Shop' },
                            { id: 'building', name_bn: 'বিশ্বসাহিত্য কেন্দ্র ভবন', name_en: 'BSK Building' },
                            { id: 'central-library', name_bn: 'কেন্দ্র লাইব্রেরি', name_en: 'Central Library' },
                            { id: 'nationwide-excellence', name_bn: 'দেশভিত্তিক উৎকর্ষ কার্যক্রম', name_en: 'Nationwide Excellence Program' },
                            { id: 'mobile-library', name_bn: 'ভ্রাম্যমাণ লাইব্রেরি', name_en: 'Mobile Library' },
                            { id: 'reading-habit', name_bn: 'পাঠাভ্যাস উন্নয়ন কর্মসূচি', name_en: 'Reading Habit Development' },
                            { id: 'book-fair', name_bn: 'ভ্রাম্যমাণ বইমেলা', name_en: 'Mobile Book Fair' },
                            { id: 'aalor-ishkool', name_bn: 'আলোর ইশকুল', name_en: 'Aalor Ishkool' },
                            { id: 'aalor-pathshala', name_bn: 'আলোর পাঠশালা', name_en: 'Aalor Pathshala' },
                            { id: 'bangalir_chinta', name_bn: 'বাঙালির চিন্তা কর্মসূচি', name_en: 'Bengali Thought Program' },
                            { id: 'primary-teacher', name_bn: 'প্রাথমিক শিক্ষকদের বই পড়া কর্মসূচি', name_en: 'Primary Teachers Reading Program' },
                            { id: 'notice', name_bn: 'বিজ্ঞপ্তি (নোটিশ ও তথ্য)', name_en: 'Announcements & Notices' },
                            { id: 'blog', name_bn: 'ব্লগ (সাহিত্য ও চিন্তা নিবন্ধ)', name_en: 'BSK Blog' }
                          ].map((pageInfo) => {
                            // Find if we already customized this page in Firestore
                        ...
                        <div className="lg:col-span-7">
                          <form onSubmit={savePageOverride} className="bg-white rounded-xl border border-[#B8862A]/20 shadow-md p-6 space-y-6">
                            ...
                            <div className="space-y-4">
                              ...
                              {/* Dedicated Nationwide Excellence Page Editor */}
                              {editingPage.id === 'nationwide-excellence' && (
                                <div className="space-y-6 pt-2 border-t border-[#B8862A]/20">
                                  {/* Hero Banner Controls */}
                                  <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/30 space-y-4">
                                    <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-2 border-b border-[#B8862A]/20 pb-2">
                                      ...
                                      <span>{language === 'bn' ? '১. হিরো ব্যানার ও স্লোগান' : '1. Hero Banner & Tagline'}</span>
                                    </h5>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div className="space-y-1">
                                        ...
                                        <input 
                                          ...
                                          placeholder="বিশ্বসাহিত্য কেন্দ্র • প্রধান ফ্ল্যাগশিপ আন্দোলন"
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        <input 
                                          ...
                                          placeholder="Bishwo Shahitto Kendro • Primary Flagship Movement"
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white"
                                        />
                                      </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-stone-700 block">{language === 'bn' ? 'সাবটাইটেল / স্লোগান (বাংলা)' : 'Subtitle / Slogan (BN)'}</label>
                                        <textarea 
                                          rows={2}
                                      ...
                                      <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-stone-700 block">{language === 'bn' ? 'সাবটাইটেল / স্লোগান (ইংরেজি)' : 'Subtitle / Slogan (EN)'}</label>
                                        <textarea 
                                          rows={2}
                                    ...
                                    <div className="space-y-1.5">
                                      <label className="text-[10px] font-bold text-stone-700 block">{language === 'bn' ? 'হিরো ব্যাকগ্রাউন্ড ছবি (URL / Upload)' : 'Hero Banner Image (URL / Upload)'}</label>
                                      <div className="flex flex-col sm:flex-row items-center gap-3">
                                        <input 
                                          ...
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-mono"
                                        />
                                        <label className="px-3 py-1.5 bg-[#2E5942] text-white rounded-lg text-xs font-bold cursor-pointer shrink-0 flex items-center gap-1">
                                          <Upload className="w-3.5 h-3.5" />
                                          <span>{language === 'bn' ? 'আপলোড' : 'Upload'}</span>
                                          <input type="file" accept="image/*" className="hidden" onChange={handleExcellenceHeroImageUpload} />
                                        </label>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        ...
                                        <span>{language === 'bn' ? '২. মূল পরিসংখ্যান ইনফো-কার্ডসমূহ (Stats)' : '2. Key Statistics Info Cards'}</span>
                                      </h5>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          const currentStats = editingPage.stats || [];
                                          setEditingPage({
                                            ...editingPage,
                                            stats: [...currentStats, { value: '১,০০০+', label_bn: 'নতুন ক্ষেত্র', label_en: 'New Metric', subtext_bn: 'বিবরণ', subtext_en: 'Details' }]
                                          });
                                        }}
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                        <span>{language === 'bn' ? 'স্ট্যাট যোগ করুন' : 'Add Stat'}</span>
                                      </button>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      {(editingPage.stats || []).map((st, stIdx) => (
                                        <div key={stIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2 relative group/st">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-stone-500 uppercase">Stat #{stIdx + 1}</span>
                                            <button
                                              type="button"
                                              onClick={() => {
                                                const updated = (editingPage.stats || []).filter((_, i) => i !== stIdx);
                                                setEditingPage({ ...editingPage, stats: updated });
                                              }}
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="Value (e.g. ১৫,০০,০০০+)" 
                                              value={st.value} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.stats || [])];
                                            ...
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.stats || [])];
                                                updated[stIdx].label_bn = e.target.value;
                                                setEditingPage({ ...editingPage, stats: updated });
                                              }}
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.stats || [])];
                                                updated[stIdx].label_en = e.target.value;
                                                setEditingPage({ ...editingPage, stats: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                            <input 
                                              type="text" 
                                              placeholder="Subtext BN" 
                                              value={st.subtext_bn || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.stats || [])];
                                                updated[stIdx].subtext_bn = e.target.value;
                                                setEditingPage({ ...editingPage, stats: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        ...
                                        <span>{language === 'bn' ? '৩. কার্যক্রমের মূল বৈশিষ্ট্যসমূহ (Highlights)' : '3. Program Core Highlights'}</span>
                                      </h5>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          ...
                                          setEditingPage({
                                            ...editingPage,
                                            highlights: [...current, { id: String(Date.now()), title_bn: 'নতুন বৈশিষ্ট্য', title_en: 'New Feature', desc_bn: 'বিবরণ', desc_en: 'Description', image: '' }]
                                          });
                                        }}
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                        <span>{language === 'bn' ? 'কার্ড যোগ করুন' : 'Add Card'}</span>
                                      </button>
                                    </div>

                                    <div className="space-y-3">
                                      {(editingPage.highlights || []).map((hl, hlIdx) => (
                                        <div key={hlIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-[#2E5942] uppercase">Highlight #{hlIdx + 1}</span>
                                            <button
                                              type="button"
                                              onClick={() => {
                                                const updated = (editingPage.highlights || []).filter((_, i) => i !== hlIdx);
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                ...
                                                updated[hlIdx].title_bn = e.target.value;
                                                setEditingPage({ ...editingPage, highlights: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.highlights || [])];
                                                updated[hlIdx].title_en = e.target.value;
                                                setEditingPage({ ...editingPage, highlights: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                            />
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            <textarea 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.highlights || [])];
                                                updated[hlIdx].desc_bn = e.target.value;
                                                setEditingPage({ ...editingPage, highlights: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                            />
                                            <textarea 
                                              rows={2}
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.highlights || [])];
                                                updated[hlIdx].desc_en = e.target.value;
                                                setEditingPage({ ...editingPage, highlights: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                            />
                                          ...
                                          <div className="flex items-center gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.highlights || [])];
                                                updated[hlIdx].image = e.target.value;
                                                setEditingPage({ ...editingPage, highlights: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-mono flex-1"
                                            />
                                            <label className="px-2.5 py-1 bg-[#2E5942] text-white rounded text-xs font-bold cursor-pointer shrink-0">
                                              Upload
                                              <input type="file" accept="image/*" className="hidden" onChange={(e) => handleExcellenceHighlightImageUpload(e, hlIdx)} />
                                            </label>
                                          </div>
                                        </div>
                                      ))}
                                  ...
                                  {/* Class Levels Controls */}
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <GraduationCap className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? '৪. শ্রেণিভিত্তিক স্তরসমূহ (Class Levels)' : '4. Class-wise Levels Breakdown'}</span>
                                      </h5>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          const current = editingPage.levels || [];
                                          setEditingPage({
                                            ...editingPage,
                                            levels: [...current, { id: String(Date.now()), level_bn: 'নতুন স্তর', level_en: 'New Level', target_group_bn: 'লক্ষ্যশ্রেণি', target_group_en: 'Target Group', books_count: '৬টি বই', desc_bn: 'বিবরণ', desc_en: 'Description', reward_bn: 'পুরস্কার', reward_en: 'Reward' }]
                                          });
                                        }}
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                        <span>{language === 'bn' ? 'স্তর যোগ করুন' : 'Add Level'}</span>
                                      </button>
                                    </div>

                                    <div className="space-y-3">
                                      {(editingPage.levels || []).map((lvl, lvlIdx) => (
                                        <div key={lvlIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-[#B8862A] uppercase">Level #{lvlIdx + 1}</span>
                                            <button
                                              type="button"
                                              onClick={() => {
                                                const updated = (editingPage.levels || []).filter((_, i) => i !== lvlIdx);
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="text-[10px] text-red-600 hover:underline font-bold cursor-pointer"
                                            >
                                              {language === 'bn' ? 'মুছুন' : 'Remove'}
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="Level Name BN" 
                                              value={lvl.level_bn} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].level_bn = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].level_en = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                            />
                                            <input 
                                              type="text" 
                                              placeholder="Books Count (e.g. ৬-৮টি বই)" 
                                              value={lvl.books_count} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].books_count = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                            />
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="Target Group BN" 
                                              value={lvl.target_group_bn} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].target_group_bn = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                            <input 
                                              type="text" 
                                              placeholder="Target Group EN" 
                                              value={lvl.target_group_en} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].target_group_en = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            <textarea 
                                              rows={2}
                                              placeholder="Reward BN" 
                                              value={lvl.reward_bn} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].reward_bn = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                            />
                                            <textarea 
                                              rows={2}
                                              placeholder="Reward EN" 
                                              value={lvl.reward_en} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.levels || [])];
                                                updated[lvlIdx].reward_en = e.target.value;
                                                setEditingPage({ ...editingPage, levels: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                            />
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        ...
                                        <span>{language === 'bn' ? '৫. ছবি গ্যালারি (Photo Gallery)' : '5. Photo Gallery Images'}</span>
                                      </h5>
                                      <button
                                        type="button"
                                        onClick={() => {
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      {(editingPage.excellence_gallery || []).map((gal, galIdx) => (
                                        <div key={galIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-stone-500">Photo #{galIdx + 1}</span>
                                            <button
                                              type="button"
                                              onClick={() => {
                                                const updated = (editingPage.excellence_gallery || []).filter((_, i) => i !== galIdx);
                                          ...
                                          <div className="flex items-center gap-2">
                                            ...
                                            <label className="px-2.5 py-1 bg-[#2E5942] text-white rounded text-xs font-bold cursor-pointer shrink-0">
                                              Upload
                                              <input type="file" accept="image/*" className="hidden" onChange={(e) => handleExcellenceGalleryImageUpload(e, galIdx)} />
                                            </label>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            ...
                                            <input 
                                              ...
                                              value={gal.caption_en} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.excellence_gallery || [])];
                                                updated[galIdx].caption_en = e.target.value;
                                                setEditingPage({ ...editingPage, excellence_gallery: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                  ...
                                  {/* Side Mini Gallery Controls */}
                                  <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/30 space-y-3">
                                    <div className="flex items-center justify-between border-b border-[#B8862A]/20 pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <ImageIcon className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? 'ডানপাশের মিনি গ্যালারি (Side Mini Gallery)' : 'Sidebar Mini Gallery Images'}</span>
                                      </h5>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          const current = editingPage.side_mini_gallery || [];
                                          setEditingPage({
                                            ...editingPage,
                                            side_mini_gallery: [...current, { image: '', caption_bn: 'মিনি গ্যালারি ক্যাপশন', caption_en: 'Mini Gallery Caption' }]
                                          });
                                        }}
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                        <span>{language === 'bn' ? 'মিনি ছবি যোগ করুন' : 'Add Side Photo'}</span>
                                      </button>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      {(editingPage.side_mini_gallery || []).map((sgal, sgalIdx) => (
                                        <div key={sgalIdx} className="p-3 bg-white rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-[#B8862A]">Side Photo #{sgalIdx + 1}</span>
                                            <button
                                              type="button"
                                              onClick={() => {
                                                const updated = (editingPage.side_mini_gallery || []).filter((_, i) => i !== sgalIdx);
                                                setEditingPage({ ...editingPage, side_mini_gallery: updated });
                                              }}
                                              className="text-[10px] text-red-600 hover:underline font-bold cursor-pointer"
                                            >
                                              {language === 'bn' ? 'মুছুন' : 'Remove'}
                                          ...
                                          <div className="flex items-center gap-2">
                                            <input 
                                              ...
                                              value={sgal.image} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.side_mini_gallery || [])];
                                                updated[sgalIdx].image = e.target.value;
                                                setEditingPage({ ...editingPage, side_mini_gallery: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-mono flex-1"
                                            />
                                            <label className="px-2.5 py-1 bg-[#2E5942] text-white rounded text-xs font-bold cursor-pointer shrink-0">
                                              Upload
                                              <input type="file" accept="image/*" className="hidden" onChange={(e) => handleExcellenceSideGalleryImageUpload(e, sgalIdx)} />
                                            </label>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="Caption BN" 
                                              value={sgal.caption_bn} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.side_mini_gallery || [])];
                                                updated[sgalIdx].caption_bn = e.target.value;
                                                setEditingPage({ ...editingPage, side_mini_gallery: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                            <input 
                                              type="text" 
                                              placeholder="Caption EN" 
                                              value={sgal.caption_en} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.side_mini_gallery || [])];
                                                updated[sgalIdx].caption_en = e.target.value;
                                                setEditingPage({ ...editingPage, side_mini_gallery: updated });
                                              }}
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                            />
                                  ...
                                  <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/30 space-y-3">
                                    <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5 border-b border-[#B8862A]/20 pb-2">
                                      <Phone className="w-4 h-4 text-[#2E5942]" />
                                      <span>{language === 'bn' ? '৬. সমন্বয়ক ও যোগাযোগ সেল (Coordinator Cell)' : '6. Coordinator & Central Contact Cell'}</span>
                                    </h5>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <input 
                                        type="text" 
                                        placeholder="Name BN" 
                                        value={editingPage.coordinator?.name_bn || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, name_bn: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                      />
                                      <input 
                                        type="text" 
                                        placeholder="Name EN" 
                                        value={editingPage.coordinator?.name_en || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, name_en: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold"
                                      />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <input 
                                        type="text" 
                                        placeholder="Designation BN" 
                                        value={editingPage.coordinator?.designation_bn || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, designation_bn: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                      />
                                      <input 
                                        type="text" 
                                        placeholder="Designation EN" 
                                        value={editingPage.coordinator?.designation_en || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, designation_en: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white"
                                      />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <input 
                                        ...
                                        value={editingPage.coordinator?.phone || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, phone: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-mono"
                                      />
                                      <input 
                                        type="text" 
                                        placeholder="Email Address" 
                                        value={editingPage.coordinator?.email || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, email: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-mono"
                                      />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <textarea 
                                        rows={2}
                                        placeholder="Office Address BN" 
                                        value={editingPage.coordinator?.office_bn || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, office_bn: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                      />
                                      <textarea 
                                        rows={2}
                                        placeholder="Office Address EN" 
                                        value={editingPage.coordinator?.office_en || ''} 
                                        onChange={(e) => setEditingPage({ ...editingPage, coordinator: { ...editingPage.coordinator, office_en: e.target.value } as any })}
                                        className="p-1.5 border border-stone-200 rounded text-xs bg-white font-sans"
                                      />
                                    </div>
                                  </div>
                              ...
                              {/* Dedicated Book Fair Page Editor */}
                              {editingPage.id === 'book-fair' && (
                                <div className="space-y-6 pt-2 border-t border-[#B8862A]/20">
                                  {/* 1. Hero Banner Controls */}
                                  <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/30 space-y-4">
                                    <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-2 border-b border-[#B8862A]/20 pb-2">
                                      <Sparkles className="w-4 h-4 text-[#B8862A]" />
                                      <span>{language === 'bn' ? '১. হিরো ব্যানার ও স্লোগান' : '1. Hero Banner & Tagline'}</span>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div className="space-y-1">
                                        ...
                                        <input 
                                          ...
                                          value={editingPage.badge_bn || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, badge_bn: e.target.value })}
                                          placeholder="বিশ্বসাহিত্য কেন্দ্র • দেশব্যাপী বইমেলা কার্যক্রম"
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white"
                                        />
                                      </div>
                                      <div className="space-y-1">
                                        ...
                                        <input 
                                          ...
                                          value={editingPage.badge_en || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, badge_en: e.target.value })}
                                          placeholder="Bishwo Shahitto Kendro • Book Fair Movement"
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white"
                                        />
                                      </div>
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <div className="flex flex-col sm:flex-row items-center gap-3">
                                        <input 
                                          ...
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-mono"
                                        />
                                        <label className="px-3.5 py-2 bg-[#2E5942] hover:bg-[#234533] text-white rounded-lg text-xs font-bold cursor-pointer shrink-0 inline-flex items-center justify-center gap-1.5 transition-colors shadow-xs">
                                          <Upload className="w-3.5 h-3.5" />
                                          <span>{language === 'bn' ? 'আপলোড' : 'Upload'}</span>
                                          <input type="file" accept="image/*" className="hidden" onChange={handleExcellenceHeroImageUpload} />
                                        </label>
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5 border-b pb-2">
                                      <BookOpen className="w-4 h-4 text-[#B8862A]" />
                                      <span>{language === 'bn' ? '২. বইমেলা কার্যক্রম পরিচিতি ও অনুচ্ছেদসমূহ' : '2. Overview & History Paragraphs'}</span>
                                    </h5>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'পরিচিতি শিরোনাম (বাংলা)' : 'Heading (BN)'}</label>
                                        <input 
                                          type="text" 
                                          value={editingPage.overview_title_bn || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, overview_title_bn: e.target.value })} 
                                          placeholder="বইমেলাঃ আলোর স্পর্শে আলোকিত সমাজ"
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-bold" 
                                        />
                                      </div>
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'পরিচিতি শিরোনাম (ইংরেজি)' : 'Heading (EN)'}</label>
                                        <input 
                                          type="text" 
                                          value={editingPage.overview_title_en || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, overview_title_en: e.target.value })} 
                                          placeholder="Book Fair: Spreading the Light of Wisdom"
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-bold" 
                                        />
                                      </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'প্রথম অনুচ্ছেদ (বাংলা)' : 'Paragraph 1 (BN)'}</label>
                                        <textarea 
                                          rows={3} 
                                      ...
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'প্রথম অনুচ্ছেদ (ইংরেজি)' : 'Paragraph 1 (EN)'}</label>
                                        <textarea 
                                          rows={3} 
                                          value={editingPage.overview_p1_en || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, overview_p1_en: e.target.value })} 
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-sans" 
                                        />
                                      </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'দ্বিতীয় অনুচ্ছেদ (বাংলা)' : 'Paragraph 2 (BN)'}</label>
                                        <textarea 
                                          rows={3} 
                                      ...
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'দ্বিতীয় অনুচ্ছেদ (ইংরেজি)' : 'Paragraph 2 (EN)'}</label>
                                        <textarea 
                                          rows={3} 
                                          value={editingPage.overview_p2_en || ''} 
                                          onChange={(e) => setEditingPage({ ...editingPage, overview_p2_en: e.target.value })} 
                                          className="w-full p-2 border border-stone-200 rounded text-xs bg-white font-sans" 
                                        />
                                      </div>
                                    </div>
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <ShieldCheck className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? '৩. কেন বিশ্বসাহিত্য কেন্দ্রের বইমেলা অনন্য?' : '3. Why is BSK Book Fair Unique?'}</span>
                                      </h5>
                                      <button 
                                        type="button" 
                                        onClick={() => {
                                          const current = editingPage.why_unique || [];
                                          setEditingPage({ ...editingPage, why_unique: [...current, { title_bn: 'নতুন বৈশিষ্ট্য', title_en: 'New Feature', desc_bn: 'বিবরণ', desc_en: 'Description' }] });
                                        }} 
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                    ...
                                    <div className="space-y-3">
                                      {(editingPage.why_unique || []).map((item: any, itemIdx: number) => (
                                        <div key={itemIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-[#2E5942]">Feature #{itemIdx + 1}</span>
                                            <button 
                                              type="button" 
                                              onClick={() => {
                                                const updated = (editingPage.why_unique || []).filter((_: any, i: number) => i !== itemIdx);
                                                setEditingPage({ ...editingPage, why_unique: updated });
                                              }} 
                                              className="text-[10px] text-red-600 hover:underline font-bold cursor-pointer"
                                            >
                                              {language === 'bn' ? 'মুছুন' : 'Remove'}
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              value={item.title_bn || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.why_unique || [])];
                                                updated[itemIdx].title_bn = e.target.value;
                                                setEditingPage({ ...editingPage, why_unique: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold" 
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.why_unique || [])];
                                                updated[itemIdx].title_en = e.target.value;
                                                setEditingPage({ ...editingPage, why_unique: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold" 
                                            />
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.why_unique || [])];
                                                updated[itemIdx].desc_bn = e.target.value;
                                                setEditingPage({ ...editingPage, why_unique: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.why_unique || [])];
                                                updated[itemIdx].desc_en = e.target.value;
                                                setEditingPage({ ...editingPage, why_unique: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <ImageIcon className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? '৬. বইমেলা ও মেধা উৎসব গ্যালারি (Photo Gallery)' : '6. Photo Gallery Images'}</span>
                                      </h5>
                                      <button 
                                        type="button" 
                                        onClick={() => {
                                          const current = editingPage.gallery || [];
                                          const updated = [...current, { image: '', caption_bn: 'স্মরণীয় মুহূর্ত', caption_en: 'Memorable Moment' }];
                                        ...
                                        <div key={galIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex justify-between items-center">
                                            ...
                                            <button 
                                              ...
                                              onClick={() => {
                                                const current = editingPage.gallery || [];
                                                const updated = current.filter((_: any, i: number) => i !== galIdx);
                                                setEditingPage({ ...editingPage, gallery: updated });
                                              }} 
                                              className="text-[10px] text-red-600 hover:underline font-bold cursor-pointer"
                                            >
                                          ...
                                          {gal.image && (
                                            <div className="relative w-full h-24 bg-stone-100 rounded-lg overflow-hidden border border-stone-200 flex items-center justify-center">
                                              <img src={gal.image} alt={`Gallery ${galIdx}`} className="w-full h-full object-cover" />
                                            </div>
                                          )}
                                          ...
                                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const current = [...(editingPage.gallery || [])];
                                                current[galIdx].image = e.target.value;
                                                setEditingPage({ ...editingPage, gallery: current });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-mono flex-1 min-w-0" 
                                            />
                                            <label className="px-3 py-1.5 bg-[#2E5942] hover:bg-[#234533] text-white rounded-lg text-xs font-bold cursor-pointer shrink-0 inline-flex items-center justify-center gap-1.5 transition-colors shadow-xs">
                                              <Upload className="w-3.5 h-3.5" />
                                              <span>{language === 'bn' ? 'আপলোড' : 'Upload'}</span>
                                              <input type="file" accept="image/*" className="hidden" onChange={(e) => handleBookFairGalleryUpload(e, galIdx)} />
                                            </label>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const current = [...(editingPage.gallery || [])];
                                                current[galIdx].caption_bn = e.target.value;
                                                setEditingPage({ ...editingPage, gallery: current });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const current = [...(editingPage.gallery || [])];
                                                current[galIdx].caption_en = e.target.value;
                                                setEditingPage({ ...editingPage, gallery: current });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                  ...
                                  {/* 7. Forms & Document Downloads */}
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <FileText className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? '৭. ফরম ও ডকুমেন্ট ডাউনলোড (Forms & Documents Download)' : '7. Forms & Document Downloads'}</span>
                                      </h5>
                                      <button 
                                        type="button" 
                                        onClick={() => {
                                          const current = editingPage.downloads || [];
                                          setEditingPage({ ...editingPage, downloads: [...current, { id: String(Date.now()), title_bn: 'নতুন ফরম / আবেদনপত্র', title_en: 'New Form / Application', file_size: '১.৫ MB (PDF)', url: '' }] });
                                        }} 
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                    ...
                                    <div className="space-y-3">
                                      {(editingPage.downloads || []).map((dl: any, dlIdx: number) => (
                                        <div key={dlIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-[#2E5942]">Document #{dlIdx + 1}</span>
                                            <button 
                                              type="button" 
                                              onClick={() => {
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            ...
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.downloads || [])];
                                                updated[dlIdx].title_en = e.target.value;
                                                setEditingPage({ ...editingPage, downloads: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold" 
                                            />
                                          ...
                                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="File Size Info (e.g. ২.৫ MB PDF)" 
                                              value={dl.file_size || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.downloads || [])];
                                            ...
                                            <div className="flex items-center gap-2">
                                              ...
                                              <label className="px-3 py-1.5 bg-[#2E5942] hover:bg-[#234533] text-white rounded-lg text-xs font-bold cursor-pointer shrink-0 inline-flex items-center justify-center gap-1.5 transition-colors shadow-xs">
                                                ...
                                                <input type="file" className="hidden" onChange={(e) => handleBookFairDownloadFileUpload(e, dlIdx)} />
                                              </label>
                                            </div>
                                          </div>
                                        </div>
                                  ...
                                  {/* 5. Book Categories & Discounts */}
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <BookOpen className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === 'bn' ? '৫. বইয়ের ক্যাটাগরি ও বিশেষ ছাড় (Categories & Discounts)' : '5. Book Categories & Discounts'}</span>
                                      </h5>
                                      <button 
                                        type="button" 
                                        onClick={() => {
                                          const current = editingPage.categories || [];
                                          setEditingPage({ ...editingPage, categories: [...current, { title_bn: 'নতুন ক্যাটাগরি', title_en: 'New Category', discount: '২৫% ছাড়', books: '১,০০০+ বই' }] });
                                        }} 
                                        className="text-[11px] font-bold text-[#2E5942] hover:underline cursor-pointer flex items-center gap-1"
                                      >
                                        <Plus className="w-3.5 h-3.5" />
                                    ...
                                    <div className="space-y-3">
                                      {(editingPage.categories || []).map((cat: any, catIdx: number) => (
                                        <div key={catIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-[#2E5942]">Category #{catIdx + 1}</span>
                                            <button 
                                              type="button" 
                                              onClick={() => {
                                                const updated = (editingPage.categories || []).filter((_: any, i: number) => i !== catIdx);
                                                setEditingPage({ ...editingPage, categories: updated });
                                              }} 
                                              className="text-[10px] text-red-600 hover:underline font-bold cursor-pointer"
                                            >
                                              {language === 'bn' ? 'মুছুন' : 'Remove'}
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              ...
                                              value={cat.title_bn || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.categories || [])];
                                                updated[catIdx].title_bn = e.target.value;
                                                setEditingPage({ ...editingPage, categories: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold" 
                                            />
                                            <input 
                                              ...
                                              onChange={(e) => {
                                                const updated = [...(editingPage.categories || [])];
                                                updated[catIdx].title_en = e.target.value;
                                                setEditingPage({ ...editingPage, categories: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white font-bold" 
                                            />
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <input 
                                              type="text" 
                                              placeholder="Discount (e.g. ২৫% ছাড়)" 
                                              value={cat.discount || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.categories || [])];
                                                updated[catIdx].discount = e.target.value;
                                                setEditingPage({ ...editingPage, categories: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                            <input 
                                              type="text" 
                                              placeholder="Books Count (e.g. ১,২০০+ বই)" 
                                              value={cat.books || ''} 
                                              onChange={(e) => {
                                                const updated = [...(editingPage.categories || [])];
                                                updated[catIdx].books = e.target.value;
                                                setEditingPage({ ...editingPage, categories: updated });
                                              }} 
                                              className="p-1.5 border border-stone-200 rounded text-xs bg-white" 
                                            />
                                  ...
                                  <div className="bg-white p-4 rounded-xl border border-stone-200 space-y-3">
                                    <div className="flex items-center justify-between border-b pb-2">
                                      <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5">
                                        <ImageIcon className="w-4 h-4 text-[#B8862A]" />
                                        <span>{language === "bn" ? "৬. বইমেলা গ্যালারি (Photo Gallery)" : "6. Photo Gallery Images"}</span>
                                      </h5>
                                      <button 
                                        type="button" 
                                        onClick={() => {
                                        ...
                                        <div key={dlIdx} className="p-3 bg-[#FAF7F2] rounded-xl border border-stone-200 space-y-2">
                                          <div className="flex justify-between items-center">
                                            ...
                                            <button 
                                              ...
                                              className="text-[10px] text-red-[#dc2626] hover:underline font-bold cursor-pointer"
                                            >
                                              {language === "bn" ? "মুছুন" : "Remove"}
                                            </button>
                                          </div>
                                  ...
                                  {/* 8. Stall Booking & Coordination Cell */}
                                  <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/30 space-y-3">
                                    <h5 className="text-xs font-bold text-[#1A1207] font-serif flex items-center gap-1.5 border-b border-[#B8862A]/20 pb-2">
                                      <Phone className="w-4 h-4 text-[#2E5942]" />
                                      <span>{language === 'bn' ? '৮. স্টল বরাদ্দ ও সমন্বয় সেল যোগাযোগ তথ্য (Stall Booking & Coordination Cell)' : '8. Stall Booking & Fair Coordination Cell Contact Info'}</span>
                                    </h5>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'অফিসের ঠিকানা (বাংলা)' : 'Office Address (BN)'}</label>
                                        <textarea 
                                          rows={2} 
                                          value={editingPage.coordination_cell?.office_bn || editingPage.coordinator?.office_bn || ''} 
                                          onChange={(e) => {
                                            const val = e.target.value;
                                            setEditingPage({
                                              ...editingPage,
                                              coordination_cell: { ...(editingPage.coordination_cell || {}), office_bn: val },
                                              coordinator: { ...(editingPage.coordinator || {}), office_bn: val }
                                            });
                                          }} 
                                          className="w-full p-1.5 border border-stone-200 rounded text-xs bg-white font-sans" 
                                        />
                                      </div>
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'অফিসের ঠিকানা (ইংরেজি)' : 'Office Address (EN)'}</label>
                                        <textarea 
                                          rows={2} 
                                          value={editingPage.coordination_cell?.office_en || editingPage.coordinator?.office_en || ''} 
                                          onChange={(e) => {
                                            const val = e.target.value;
                                            setEditingPage({
                                              ...editingPage,
                                              coordination_cell: { ...(editingPage.coordination_cell || {}), office_en: val },
                                              coordinator: { ...(editingPage.coordinator || {}), office_en: val }
                                            });
                                          }} 
                                          className="w-full p-1.5 border border-stone-200 rounded text-xs bg-white font-sans" 
                                        />
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'ফোন / মোবাইল নম্বর' : 'Phone / Mobile Number'}</label>
                                        <input 
                                          type="text" 
                                          value={editingPage.coordination_cell?.phone || editingPage.coordinator?.phone || ''} 
                                          onChange={(e) => {
                                            const val = e.target.value;
                                            setEditingPage({
                                              ...editingPage,
                                              coordination_cell: { ...(editingPage.coordination_cell || {}), phone: val },
                                              coordinator: { ...(editingPage.coordinator || {}), phone: val }
                                            });
                                          }} 
                                      ...
                                      <div>
                                        <label className="text-[10px] font-bold text-stone-700 block mb-1">{language === 'bn' ? 'ইমেইল এড্রেস' : 'Email Address'}</label>
                                        <input 
                                          type="email" 
                                          value={editingPage.coordination_cell?.email || editingPage.coordinator?.email || ''} 
                                          onChange={(e) => {
                                            const val = e.target.value;
                                            setEditingPage({
                                              ...editingPage,
                                              coordination_cell: { ...(editingPage.coordination_cell || {}), email: val },
                                              coordinator: { ...(editingPage.coordinator || {}), email: val }
                                            });
                                          }} 
                                          className="w-full p-1.5 border border-stone-200 rounded text-xs bg-white font-mono" 
                                        />
                                  ...
                                  <div className="space-y-4">
                                    {editingPage.sections.map((section, secIdx) => (
                                      <div key={secIdx} className="p-4 bg-stone-50 border border-stone-200/80 rounded-xl space-y-3 relative group/sec">
                                          ...
                                          <div className="flex items-center gap-2">
                                            {secIdx > 0 && (
                                              <button
                                                type="button"
                                                onClick={() => {
                                                  if (window.confirm(language === 'bn' ? 'আপনি কি এই সেকশনটি মুছে ফেলতে চান?' : 'Are you sure you want to delete this section?')) {
                                                    const updatedSections = editingPage.sections.filter((_, sI) => sI !== secIdx);
                                                    setEditingPage({ ...editingPage, sections: updatedSections });
                                                  }
                                                }}
                                                className="text-[10px] text-red-500 hover:text-red-700 font-semibold cursor-pointer font-sans opacity-0 group-hover/sec:opacity-100 transition duration-200"
                                              >
                                                {language === 'bn' ? 'সেকশন মুছুন' : 'Delete Section'}
                                              </button>
                                            )}
                                          </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                          <div className="space-y-1">
                                            <label className="text-[9px] font-bold text-stone-500 block">{language === 'bn' ? 'সেকশন শিরোনাম (বাংলা বা ইংরেজি)' : 'Section Title (BN or EN)'}</label>
                                            <input 
                                              type="text" 
                                              value={section.title || ''} 
                                              onChange={(e) => {
                                              ...
                                              placeholder={language === 'bn' ? 'যেমন: পরিচিতি / ইতিহাস' : 'e.g., Introduction'} 
                                              className="w-full p-2 border border-stone-200 rounded text-xs bg-white"
                                            />
                                          </div>
                                        </div>

                                        {/* General image upload for any section under programs */}
                                        <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#B8862A]/20 space-y-3 mb-3">
                                          <label className="text-[10px] font-bold text-stone-700 block font-serif">
                                            {language === 'bn' ? 'সেকশন এর ছবি (Photo/Illustration)' : 'Section Photo/Illustration'}
                                          </label>
                                          <div className="flex flex-col sm:flex-row items-center gap-4">
                                            <div className="w-16 h-16 rounded-xl overflow-hidden border border-[#B8862A] bg-white flex items-center justify-center shrink-0">
                                              {section.image ? (
                                                <img 
                                                  src={section.image} 
                                                  className="w-full h-full object-cover" 
                                            ...
                                            <div className="space-y-1.5 w-full">
                                              ...
                                              <p className="text-[9px] text-stone-500">
                                                {language === 'bn' ? 'প্রস্তাবিত সাইজ: সর্বোচ্চ ২ মেগাবাইট।' : 'Recommended size: max 2MB.'}
                                              </p>
                                            </div>
                                          </div>
                                        </div>
                                            ...
                                            <div key={paraIdx} className="space-y-1">
                                              <div className="flex items-center justify-between">
                                                <span className="text-[9px] font-bold text-stone-400 uppercase tracking-wider">
                                                  {language === 'bn' ? `প্যারাগ্রাফ #${paraIdx+1}` : `Paragraph #${paraIdx+1}`}
                                                </span>
                                                {section.content.length > 1 && (
                                                  <button
                                                    type="button"
                ...
                {activeTab === 'blocks' && (
                  <div className="space-y-6">
                    <div className="bg-white p-5 rounded-2xl border border-[#B8862A]/20 shadow-xs">
                      <h3 className="text-lg font-bold text-stone-900 font-serif">
                        {language === 'bn' ? '৪. হোমপেজ টেক্সট ও কন্টেন্ট ব্লকসমূহের কপি' : '4. Homepage Rich Content Blocks Copy'}
                      </h3>
                      <p className="text-xs text-stone-500 font-sans mt-1">
                        {language === 'bn' 
                          ? 'দ্বিতীয় স্লাইডারের নিচে অবস্থিত বিভিন্ন কন্টেন্ট ব্লক এবং টেক্সট এডিট সারণী।' 
                          : 'Update single-instance headings, quotes, buttons, and blocks of different home widgets.'}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2 mt-4 border-t pt-4">
                        {[
                          ...
                          { id: 'founder', label_bn: 'প্রতিষ্ঠাতা পরিচিতি', label_en: 'Founder Biography' },
                    ...
                    {activeSubBlock === 'intro' && (
                      <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-md space-y-4">
                        ...
                        <div className="space-y-4 font-sans">
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-stone-700 block">{language === 'bn' ? 'ভূমিকা বাণী (বাংলা)' : 'Introductory Quote (Bangla)'}</label>
                            <textarea
                              rows={4}
                          ...
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-stone-700 block">{language === 'bn' ? 'ভূমিকা বাণী (ইংরেজি)' : 'Introductory Quote (English)'}</label>
                            <textarea
                              rows={4}
                    ...
                    {/* Subblock: Founder Spotlight */}
                    {activeSubBlock === 'founder' && (
                      <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-md space-y-4">
                        <div className="border-b pb-2 flex justify-between items-center">
                          <div className="space-y-0.5">
                            <h4 className="font-bold text-stone-900 font-serif">{language === 'bn' ? 'অধ্যাপক আবদুল্লাহ আবু সায়ীদ প্রোফাইল' : 'Prof. Abdullah Abu Sayeed Spotlight'}</h4>
                            <span className="text-[10px] text-[#B8862A] font-mono block">🖼️ {language === 'bn' ? 'ছবি সুপারিশকৃত সাইজ: ৪০০x৪০০ পিক্সেল (১:১ স্কয়ার)' : 'Image Recommended spec: 400x400 px (1:1 square ratio)'}</span>
                          </div>
                          {!founderBlock && (
                            <button 
                              type="button"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans text-xs">
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'ম্যাগসেসে অ্যাওয়ার্ড বিবরণী (বাংলা)' : 'Badge 1 (Bangla)'}</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'ম্যাগসেসে অ্যাওয়ার্ড বিবরণী (ইংরেজি)' : 'Badge 1 (English)'}</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'একুশে পদক বিবরণী (বাংলা)' : 'Badge 2 (Bangla)'}</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'একুশে পদক বিবরণী (ইংরেজি)' : 'Badge 2 (English)'}</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'ইউনেস্কো পদক বিবরণী (বাংলা)' : 'Badge 3 (Bangla)'}</label>
                            <input 
                              type="text" 
                              value={founderBlock?.badge3_bn || ''} 
                              onChange={(e) => setFounderBlock({ ...founderBlock, badge3_bn: e.target.value })}
                              className="w-full p-2 border border-stone-200 rounded-lg"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'ইউনেস্কো পদক বিবরণী (ইংরেজি)' : 'Badge 3 (English)'}</label>
                            <input 
                              type="text" 
                              value={founderBlock?.badge3_en || ''} 
                              onChange={(e) => setFounderBlock({ ...founderBlock, badge3_en: e.target.value })}
                              className="w-full p-2 border border-stone-200 rounded-lg"
                            />
                          </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <input 
                              ...
                              placeholder="জীবনী ও সাক্ষাৎকার পড়ুন"
                              className="w-full p-2 border border-stone-200 rounded-lg"
                            />
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <input 
                              ...
                              placeholder="Read Biography & Interviews"
                              className="w-full p-2 border border-stone-200 rounded-lg"
                            />
                          </div>

                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'দর্শনার্থী/ভিউয়ার কাউন্ট' : 'Visitor count'}</label>
                            <input 
                              type="text" 
                          ...
                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'হোমপেজে ছবির অবস্থান' : 'Portrait Image Position on Homepage'}</label>
                            <select
                            ...
                            <span className="text-[10px] text-stone-400 block">{language === 'bn' ? '* ডেক্সটপ স্ক্রিনে ছবি ডানে নাকি বামে থাকবে তা সিলেক্ট করুন' : '* Select whether portrait image is positioned Left or Right on desktop screens'}</span>
                          </div>

                          <div className="space-y-1.5">
                            <label className="font-bold text-stone-700 block">{language === 'bn' ? 'প্রতিষ্ঠাতার ছবি URL' : 'Portrait Image URL'}</label>
                            <div className="flex gap-2">
                              <input 
                                type="text" 
                              ...
                              <button
                                type="button"
                                onClick={() => openImageResizer('square', (resizedUrl) => {
                                  setFounderBlock({ ...founderBlock, image: resizedUrl });
                                })}
                            ...
                            <span className="text-[10px] text-stone-400 block">{language === 'bn' ? '* বর্গাকার প্রতিকৃতির জন্য ১:১ সাইজ (৪০০x৪০০)' : '* Recommended square ratio 1:1 (400x400 px)'}</span>
                          </div>
                        </div>
                        ...
                        <div className="pt-2 flex justify-end">
                          ...
                          >
                            <Save className="h-3.5 w-3.5" />
                            <span>{language === 'bn' ? 'রক্ষণ করুন' : 'Save Biography'}</span>
                          </button>
                        </div>
                      </div>
                    )}
                    ...
                    {activeSubBlock === 'portals' && (
                      <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-md space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-sans">
                          ...
                          <div className="p-4 rounded-xl border border-[#B8862A]/20 bg-stone-50/50 space-y-4">
                            ...
                            <div className="space-y-3.5">
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="space-y-1">
                                  <label className="text-[11px] font-bold text-stone-600 block">{language === 'bn' ? 'শিরোনাম (En)' : 'Title (En)'}</label>
                                  <input 
                                    type="text" 
                              ...
                              <div className="space-y-1">
                                <label className="text-[11px] font-bold text-stone-600 block">{language === 'bn' ? 'টুলটিপ/পরিচিতি (বাংলা)' : 'Tooltip (Bangla)'}</label>
                                <input 
                                  type="text" 
                              ...
                              <div className="space-y-1">
                                <label className="text-[11px] font-bold text-stone-600 block">{language === 'bn' ? 'টুলটিপ/পরিচিতি (En)' : 'Tooltip (En)'}</label>
                                <input 
                                  type="text" 
                              ...
                              <div className="space-y-1.5">
                                ...
                                <div className="flex gap-2">
                                  <input 
                                    ...
                                    placeholder="/assets/IMGS/..."
                                    className="flex-1 text-xs p-2 border border-stone-200 rounded-lg bg-white" 
                                  />
                                  <button
                                    type="button"
                                    onClick={() => openImageResizer('landscape', (resizedUrl) => {
                                      const updated = { ...portalsBlock };
                                      if (!updated.bcrs) updated.bcrs = {};
                                      updated.bcrs.bgImage = resizedUrl;
                                      setPortalsBlock(updated);
                                    })}
                                    className="px-2.5 bg-stone-200 hover:bg-stone-300 text-[#2E5942] text-xs font-bold rounded-lg transition cursor-pointer"
                                  >
                                    <Upload className="h-3.5 w-3.5" />
                                  </button>
                                </div>
                              ...
                              <div className="space-y-1.5 p-2.5 bg-stone-100 rounded-lg border">
                                <div className="flex justify-between items-center text-[11px] font-bold text-stone-600">
                                  <span>{language === 'bn' ? 'অন্ধকার ওভারলে অপাসিটি (কন্ট্রাস্ট বৃদ্ধির জন্য):' : 'Contrast Overlay Opacity:'}</span>
                                  <span className="text-[#B8862A] font-sans">{portalsBlock?.bcrs?.opacity ?? 70}%</span>
                                </div>
                                <input 
                                  type="range" 
                                  min="10" 
                          ...
                          <div className="p-4 rounded-xl border border-[#2E5942]/20 bg-stone-50/50 space-y-4">
                            <div className="border-b pb-1.5 flex items-center gap-1.5">
                              <span className="text-sm font-bold text-[#2E5942]">২. আলোর পাঠশালা (Alor Pathshala)</span>
                            </div>
                            ...
                            <div className="space-y-3.5">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    ...
                                    onClick={() => openImageResizer('landscape', (resizedUrl) => {
                                      const updated = { ...portalsBlock };
                                      if (!updated.alor) updated.alor = {};
                                      updated.alor.bgImage = resizedUrl;
                                      setPortalsBlock(updated);
                                    })}
                                    className="px-2.5 bg-stone-200 hover:bg-stone-300 text-[#2E5942] text-xs font-bold rounded-lg transition cursor-pointer"
                                  >
                              ...
                              <div className="space-y-1.5 p-2.5 bg-stone-100 rounded-lg border">
                                <div className="flex justify-between items-center text-[11px] font-bold text-stone-600">
                                  <span>{language === 'bn' ? 'অন্ধকার ওভারলে অপাসিটি (কন্ট্রাস্ট বৃদ্ধির জন্য):' : 'Contrast Overlay Opacity:'}</span>
                                  <span className="text-[#B8862A] font-sans">{portalsBlock?.alor?.opacity ?? 70}%</span>
                                </div>
                                <input 
                                  type="range" 
                                  min="10" 
                        ...
                        <div className="pt-3 border-t flex justify-end">
                          <button
                            ...
                            className="px-5 py-2.5 bg-[#2E5942] hover:bg-[#1C3E2D] text-white text-xs font-bold rounded-lg shadow-sm hover:scale-102 transition flex items-center gap-1.5 cursor-pointer"
                          >
                ...
                {activeTab === 'contact' && (
                  <div className="space-y-6 animate-fade-in text-stone-800">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs">
                      <div className="flex items-center justify-between border-b border-stone-100 pb-4 mb-4">
                        <h4 className="text-xs font-extrabold text-stone-700 uppercase tracking-wider font-sans">
                          {language === 'bn' ? 'আগত বার্তা ও যোগাযোগের ইনবক্স' : 'Inquiries & Messages Inbox'}
                        </h4>
                        <span className="bg-stone-100 text-stone-700 text-[10px] px-2 py-0.5 rounded-full font-mono font-bold">
                          {inquiries.length} {language === 'bn' ? 'টি বার্তা' : 'messages'}
                        </span>
                      </div>
                      ...
                      <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
                        {inquiries.length === 0 ? (
                          <div className="p-12 text-center text-stone-400 text-xs font-sans">
                            {language === 'bn' ? 'কোনো মেসেজ বা বার্তা পাওয়া যায়নি।' : 'No message submissions found.'}
                          </div>
                        ) : (
                          inquiries.map((inq) => {
                            ...
                            return (
                              <div key={inq.id} className="p-4 bg-stone-50 border border-stone-100 rounded-xl flex flex-col md:flex-row md:items-start justify-between gap-4 font-sans text-xs">
                                <div className="space-y-2 text-left min-w-0 flex-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className="font-bold text-stone-900 text-sm">{inq.name}</span>
                                    <span className="text-stone-400 font-mono">({inq.email})</span>
                                    <span className="bg-amber-100 text-amber-800 px-2 py-0.5 border border-amber-200 rounded-full text-[9px] font-bold uppercase font-mono">
                                      {inq.type === 'contact' ? (language === 'bn' ? 'যোগাযোগ' : 'General Inquiry') : inq.type}
                                    </span>
                                    <span className="text-stone-400 font-mono text-[10px]">📅 {dateStr}</span>
                                  </div>
                                  <p className="text-stone-700 leading-relaxed font-medium bg-white p-3 border border-stone-100 rounded-xl whitespace-pre-wrap font-sans text-left">
                                    {inq.message}
                                  </p>
                                </div>
